/**
 *
 * Copyright 2010, Charlton Technology
 *
 * Created: March 19, 2010
 * @author Tim Arland <tca@charltontechnology.net>
 */
...
const char* Version = "v0.26";
bool        Alarm   = false;
int         Pid     = 0;
HexApp    * happ    = new HexApp();
...
void version()
{
    std::cout << "gktrace " << Version
              << ", Copyright (C) 2010, Gaikai.com" << std::endl
...
#   endif
    ...
    if ( dstaddr == 0 ) {
        ...
        //msg << std::endl << "Invalid target " << host << std::endl << std::endl;
...
#   ifdef USE_HEXES
    HexPanel * mainPanel = happ->createPanel("gktrace", LINES-1, COLS, 1, 0);
    mainPanel->setBorderActiveColor(HEX_GREEN);
    mainPanel->setDrawTitle(false);

    std::string top = "gktrace ";
    top.append(Version);
    happ->print(0, 1, top, HEX_WHITE, HEX_BOLD);
    happ->setFocus(mainPanel);

    TextList & tlist = mainPanel->getTextList();
#   endif
...
*/
    if ( icmp )
        msg << "Sending ICMP datagrams to ";
    else
        msg << "Sending UDP datagrams (" << size << " bytes) to ";
    msg << dstname << ". count = " << count << " interval = " << interval
        << " milliseconds" << std::endl;
    mainPanel->addText(msg.str());
    msg.seekp(std::ios::beg);
    ...
    //printStatHeader();
    mainPanel->addText(getStatHeader());
                        ...
                        if ( retry < 3 ) {
                            if ( retry == 0 ) {
                                ...
                                std::cout << std::setw(2) << ttl << ": "
                                    << std::setw(15) << CidrUtils::ntop(pdata.ipaddr)
                                    << "  <" << pdata.seq << ">"
